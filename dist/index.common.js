...
var Notification = vue.defineComponent({
    ...
    emits: ['close'],
    setup(props, { emit }) {
        const context = vue.inject(contextKey, { group: '', position: 'top' });
        const notifications = vue.ref([]);
        const notificationsByGroup = vue.computed(() => notifications.value.filter((n) => n.group === context.group));
        const sortedNotifications = vue.computed(() => {
